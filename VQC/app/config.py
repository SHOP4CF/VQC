...
def configuration():
    """
    This method updates our Analysis object if it get POST message and when
    obtain GET it shows all configurations on  the screen we could do it in
    more readable HTML format, or even create some HTML page in which we can
    ...
    if request.method == "GET":
        ...
        if os.path.isfile(config_path):
            ...
            msg = f"App.Config: Tried to update configs with {config_path}" \
                  f"but the file does not exits, returned code: 400"
            debug_help.log_fiware_resp_exception(
                logger,
                msg,
                level="error",
