...
class Analysis:
    ...
    def __init__(self, config):
        ...
        if not os.path.isdir(self.path):
            os.mkdir(self.path)
            logger.warning(
                "App.Analysis: Provided path for saving inference had not "
                "existed, we created it as: {self.path} make sure that you "
                "want to save results there "
            )

        self.save_all = self.config["interface"]["save_all"]
        """
        Just sanity check if we have our db connected if yes we initialize our
        box generator. For some reason someone may want to not use boxes, one
        can add it to config easily.
        """

        """box_generator is an util used only in template matching it
        potentially can improve its efficiency and accuracy """
        box_generator_config = self.config["utils"]["box_generator"]
        box_generator = utils.BoxGenerator(
            **box_generator_config
        )
        # during init we will read all previously saved boxes, and during
        # template matching we will try to collect more boxes
        ...
        self.methods = {
            ...
            "template_matching": methods.TemplateMatcher(
                **methods_config["template_matching"],
                box_generator=box_generator
            ),
        }

        # In this dict we will store templates for later usage not to load
        # them constantly
        self.templates_dict = {}
        ...
        self.ROI_cropper = utils.ROICropper(**cropper_config)

        debug_help.init_fiware_responder(config["interface"])

    def set_config(self, config_path: str) -> None:
        """
        At first, we again read our configs and then run set_params method in
        our components passing appropriate configs
        """
    ...
    def analyse(self, template_path: str, pcb_path: str):
        """
        Whole inspection:
        Get template and inspected image from given paths, if files do not
        exist raise an exception and log message
        ...
        Return:
            result (currently str but may be int or whatever): defected or not
        """
        to_inspect_img = cv.imread(pcb_path, cv.IMREAD_GRAYSCALE)
        template_img = cv.imread(template_path, cv.IMREAD_GRAYSCALE)

        """
        Now I assume that if I see template for the first time I'll save the
        image in the dictionary so that when I see the same path again I'll
        just take it from the dictionary
        """
        ...
        if to_inspect_img is None or template_img is None:
            msg_param = f"template: {template_path}"
            if template_img is not None:
                msg_param += f"image to be inspected: {pcb_path}"

            msg = f'App.Analysis: Wrong path to: ' \
                  f'{msg_param}, as imread returns None '
        ...
        """
            Pipeline starts here. We perform cropping at first if set in config
            -> Alignment is performed always -> Histogram equalization if set
            -> We check whether pipeline was successful (Basically the only
                thing that may go wrong is that sizes may not be equal if
                alignment is deleted from here)
        """
        ...
        if template.shape != to_inspect_img.shape:
            msg = """App.Analysis: Shapes of images does not match, Check if
            you have Aligner in your pipeline! """
            debug_help.log_fiware_resp_exception(
                logger,
                msg,
                level="error",
